...
app.get("/api/votos", async (req, res) => {
  ...
  try {
    ...
    const [rows] = await connection.execute(
      "SELECT opcao_nome, total_votos FROM tbl_votos"
    );
...
app.post("/api/votar/:opcao", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: `Voto para ${opcao} registrado com sucesso!` });
  } catch (error) {
...
app.listen(port, () => {
  ...
  console.log(
    "Sirva o frontend abrindo http://localhost:3000 no seu navegador."
  );
});
